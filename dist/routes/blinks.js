...
blinkRouter.get("/actions/affiliate-mint", async (req, res) => {
...
blinkRouter.get("/hello", async (_req, res) => {
    try {
...
blinkRouter.post('/actions/affiliate-mint', async (req, res) => {
    try {
        console.log(req);
        ...
        const [campaignPda] = await PublicKey.findProgramAddressSync([Buffer.from("nft_campaign"), Buffer.from(campaign.toString()), nftMint.toBuffer()], programId);
        const [affiliateLinkPda] = await PublicKey.findProgramAddressSync([
            Buffer.from("affiliate_link"),
            influencerKey.toBuffer(),
            Buffer.from(campaign),
        ], programId);
        const nftEscrowPda = (await PublicKey.findProgramAddressSync([Buffer.from("nft_escrow"), Buffer.from(campaign), nftMint.toBuffer()], programId))[0];
        const buyerTokenAccount = await getAssociatedTokenAddressSync(nftMint, buyer);
        const escrowPdaNftTokenAccount = await getAssociatedTokenAddressSync(nftMint, nftEscrowPda, true, // allowOwnerOffCurve = true for PDA
        TOKEN_PROGRAM_ID);
        let [marketplaceAuthority] = PublicKey.findProgramAddressSync([Buffer.from("marketplace_authority")], program.programId);
...
blinkRouter.get("/nft-image", async (req, res) => {
    try {
